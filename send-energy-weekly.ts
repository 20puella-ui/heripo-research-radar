  ...
  for (const source of sources) {
    try {
      ...
      const items = feed.items
        ...
        .filter(item => item.pubDateObj >= sevenDaysAgo)
        .slice(0, 3);
...
function buildArticleBlock(article: Article, index: number): string {
  const sourceTag = '<div style="display:inline-block;background:#ff6b35;color:white;padding:3px 10px;border-radius:12px;font-size:11px;font-weight:bold;margin-bottom:8px;">' + article.source + '</div>';
  const titleTag = '<h3 style="margin:8px 0;color:#333;font-size:16px;line-height:1.4;">' + (index + 1) + '. ' + article.title + '</h3>';
  const summaryTag = '<p style="color:#666;line-height:1.6;margin:10px 0;font-size:14px;">' + article.summary + '</p>';
  
  const linkStyle = 'color:#ff6b35;text-decoration:none;font-weight:bold;font-size:13px;display:inline-block;margin-top:8px;';
  const linkTag = '' + article.link + '원문 보기 →</a>';
  
  const wrapperStyle = 'margin:25px 0;padding:20px;border-left:4px solid #ff6b35;background:#fafafa;border-radius:6px;';
  return '<div style="' + wrapperStyle + '">' + sourceTag + titleTag + summaryTag + linkTag + '</div>';
}
...
function buildEmailHtml(articles: Article[]): string {
  ...
  const articlesHtml = articles.map((a, i) => buildArticleBlock(a, i)).join('');
  const sourceStats = Object.entries(bySource)
    .map(([source, count]) => '<li style="margin:4px 0;">' + source + ': ' + count + '건</li>')
    .join('');

  return '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"></head>' +
    '<body style="font-family:-apple-system,sans-serif;max-width:640px;margin:0 auto;padding:20px;background:#f5f5f5;">' +
    ...
    '<p style="margin:0;color:#333;font-size:14px;">안녕하세요 <strong>챠에린님</strong>, 이번 주 에너지 산업 주요 소식 <strong>' + articles.length + '건</strong>을 전해드립니다.</p>' +
    '<div style="margin-top:10px;color:#666;font-size:12px;"><strong>소스별 수집:</strong><ul style="margin:5px 0;padding-left:20px;">' + sourceStats + '</ul></div>' +
    '</div>' +
    articlesHtml +
